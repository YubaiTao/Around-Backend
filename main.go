...
const (
	...
	ES_URL = "http://35.185.80.255:9200"
	BUCKET_NAME = "around_backend_post"
...
var (
	...
	GCS_BUCKET          = BUCKET_NAME //os.Getenv("GCS_BUCKET")
)
...
//}
//type Post struct {
//	// `json:"user"` is for the json parsing of this User field. Otherwise,
...
//	fmt.Println("Received one request for search")
//	lat, _ := strconv.ParseFloat(r.URL.Query().Get("lat"), 64)
//	lon, _ := strconv.ParseFloat(r.URL.Query().Get("lon"), 64) // range is optional
//	ran := DISTANCE
//	if val := r.URL.Query().Get("range"); val != "" {
//		ran = val + "km" }
